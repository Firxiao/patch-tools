...
print colored("*********************************summary job start************************************************************",'blue')
...
if 'linux' in nodegroup:
    ...
    if salt_version < "2017.7.0":
      job_id = local.cmd_async(nodegroup,'win_wua.list_updates',kwarg={'summary':'true'},expr_form='nodegroup')
    else:
      job_id = local.cmd_async(nodegroup,'win_wua.list_updates',kwarg={'summary':'true'},tgt_type='nodegroup')
...
#      print(len(summary[minion]))
      ...
      if 'linux' in nodegroup:
        if len(summary[minion]) == 0:
          ...
          report.append((s_minion,good_minions[minion]['ip'],"Total:%d"%(len(summary[minion]))))
      elif 'win' in nodegroup:
        if isinstance(summary[minion], dict):
            report.append((s_minion,good_minions[minion]['ip'],"Total:"+str(summary[minion]['Total'])))
        else:
            info = "System is updated"
            report.append((s_minion,good_minions[minion]['ip'],info))
...
while (i <= len(good_minions)):
    ...
    if i == len(good_minions):
        ...
        print_report(job_id)
        break
print colored("*********************************summary job end*************************************************************",'blue')
